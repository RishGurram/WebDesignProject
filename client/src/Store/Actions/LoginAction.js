...
export const signUpUser = (payload) => {
    return async(dispatch) => {
        try{
            const url = 'http://localhost:9003/users';
...
export const handleUserVerification = (payload) => {
    return async(dispatch) => {
        ...
        try{
            const url = 'http://localhost:9003/users/verify-security/'+username+'?securityQuestion='+payload.securityQuestion+'&&securityAnswer='+payload.securityAnswer;
...
export const loginUser = (payload) => {
    return async (dispatch) => {
        let username = payload.username;
        try {
            const url = 'http://localhost:9003/users/login/' + username;
            const response = await HTTP.post(url, payload)
...
export const updateUserDetails = (payload, callingComponent) => {
    return async (dispatch) => {
        ...
        try {
            const url = 'http://localhost:9003/users/' + username;
...
export const updateUserEventDetails = (uuid, payload, callingComponent) => {
    return async (dispatch) => {
        try {
            const url = 'http://localhost:9003/users/save-event/' + uuid;
            const response = await HTTP.put(url, payload)
            if(response.status===200){
                sessionStorage.setItem("user",JSON.stringify(response.data.user));
            }
...
export const updateUserInterestedEventDetails = (uuid, payload, callingComponent) => {
    return async (dispatch) => {
        try {
            const url = 'http://localhost:9003/users/interested-event/' + uuid;
            const response = await HTTP.put(url, payload)
            if(response.status===200){
                sessionStorage.setItem("user",JSON.stringify(response.data.user));
            }
...
export const updateUserEventUnregisterDetails = (uuid, payload, callingComponent) => {
    return async (dispatch) => {
        try {
            const url = 'http://localhost:9003/users/unregister-event/' + uuid;
            const response = await HTTP.put(url, payload)
            if(response.status===200){
                sessionStorage.setItem("user",JSON.stringify(response.data.user));
            }
...
export const updateUserEventUnbookmarkDetails = (uuid, payload, callingComponent) => {
    return async (dispatch) => {
        try {
            const url = 'http://localhost:9003/users/unbookmark-event/' + uuid;
            const response = await HTTP.put(url, payload)
            if(response.status===200){
                sessionStorage.setItem("user",JSON.stringify(response.data.user));
            }
